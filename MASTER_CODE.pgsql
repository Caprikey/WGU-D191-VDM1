...
-- Master code builds all schemas, stores procedures, and functions necessary for ETL process. 

-- Master code will be placed into the marking schema to allow for automation of a full ETL rebuild. 
...
CREATE OR REPLACE PROCEDURE staging.vdm1_stage1()
    ...
    BEGIN 
        ...
        -- Secondary copy has been created in staging that runs at the beginning of vdm1_stage 2. 
...
CREATE OR REPLACE PROCEDURE staging.vdm1_stage2()
	...
	BEGIN
        ...
        -- RUN CUSTOMER STATUS AMEND FUNCTION ON vdm1_stage1_CUSTOMER Table to amend the activebool column
...
CREATE OR REPLACE PROCEDURE staging.vdm1_stage3()
	...
	BEGIN 
		...
		PERFORM staging.f_vdm1_stage3_tablechanges();
...
CREATE OR REPLACE PROCEDURE staging.vdm1_stage4()
	...
	BEGIN 
		...
		PERFORM staging.f_vdm1_stage4_calc_update_cat_rent_count();
		...
		PERFORM staging.vdm1_stage4_calc_update_film_inventory_flag_inspection();
		...
		PERFORM staging.f_vdm1_stage4_calc_update_film_category_rank();
...
CREATE OR REPLACE FUNCTION staging.f_vdm1_stage2_customer_category()
	...
	BEGIN 
		...
		SELECT 
			  a.customer_id
			, b.category_id
		FROM staging.vdm1_stage2_customer a
...
CREATE OR REPLACE FUNCTION staging.f_vdm1_stage3_create_table_category_count_init()
	...
	LANGUAGE plpgsql
	AS
    $vdm1_stage3_create_table_cat_count_init$
...
CREATE TABLE staging.vdm1_stage3_customer_watch_history_details AS (
	...
	GROUP BY
		a.customer_id, b.city_id, b.country_id, a.inventory_id, a.film_id, a.category_id, a.rental_date, a.return_date
	ORDER BY 
		a.customer_id
...
CREATE OR REPLACE FUNCTION staging.f_vdm1_stage3_tablechanges()
	RETURNS VOID
	LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION staging.f_vdm1_stage4_transform_filmlength_int2vchar(
	...
	RETURNS VARCHAR
	LANGUAGE plpgsql
	AS $vmd1_stage4_filmlength_int2vchar$
	...
	END;
$vmd1_stage4_filmlength_int2vchar$;
...
CREATE OR REPLACE FUNCTION staging.f_vdm1_stage4_calc_update_film_rent_count()
	RETURNS VOID
	LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION staging.vdm1_stage4_calc_update_film_inventory_flag_inspection()
	RETURNS VOID
	LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION staging.f_vdm1_stage4_calc_update_film_category_rank()
	RETURNS VOID
	LANGUAGE plpgsql
	AS $vdm1_stage4_calc_update_film_category_rank$
	
	BEGIN
	
	
		WITH get_film_category_rank AS (
			SELECT 
				film_id,
				category_id,
		...
		SET film_category_rank = get_film_category_rank.film_cat_rental_order

		FROM get_film_category_rank

		WHERE
			a.film_id = get_film_category_rank.film_id
				AND
			a.category_id = get_film_category_rank.category_id;
			
	END;
$vdm1_stage4_calc_update_film_category_rank$;
...
CREATE OR REPLACE FUNCTION staging.f_vdm1_stage4_calc_insert_failed_returns_v2()
	...
	BEGIN  
		...
		SELECT
			...
			, store_id
			, rental_date
			, staging.f_vdm1_stage4_calc_expected_return_date(a.film_id::int, rental_date) as expected_return_date
            , (SELECT AGE('2007-01-01', staging.f_vdm1_stage4_calc_expected_return_date(a.film_id::int, rental_date))) as age
...
CREATE OR REPLACE FUNCTION staging.f_vdm1_stage4_calc_update_customer_watch_history_row_number()
	...
	BEGIN
		...
		UPDATE staging.vdm1_stage4_customer_watch_history a
