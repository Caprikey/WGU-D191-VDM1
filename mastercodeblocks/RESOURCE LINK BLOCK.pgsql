-- RESOURCE LINK BLOCK

-- #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####      
-- #### #### #### #### #### #### #### #### #### #### #### ####        RESOURCE LINKS START        #### #### #### #### #### #### #### #### #### #### #### #### ####      
-- #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####                           

-- REFERENCE LINKS

-- https://www.geeksforgeeks.org/postgresql-if-statement/
-- https://www.postgresql.org/docs/current/plpgsql-errors-and-messages.html
-- https://www.enterprisedb.com/postgres-tutorials/how-raise-errors-and-report-messages-within-stored-procedures-and-functions

-- https://www.2ndquadrant.com/en/blog/postgresql-10-identity-columns/
-- https://www.postgresql.org/docs/current/sql-altertable.html
-- https://www.postgresqltutorial.com/postgresql-tutorial/postgresql-primary-key/
-- https://stackoverflow.com/questions/2944499/how-to-add-an-auto-incrementing-primary-key-to-an-existing-table-in-postgresql
-- https://www.cybertec-postgresql.com/en/sequences-gains-and-pitfalls/
-- https://popsql.com/learn-sql/postgresql/how-to-alter-sequence-in-postgresql

-- https://www.postgresqltutorial.com/postgresql-tutorial/postgresql-check-constraint/#:~:text=The%20CHECK%20constraint%20uses%20a,issue%20a%20constraint%20violation%20error.
-- 
-- https://www.postgresql.org/docs/8.2/sql-altertable.html
-- 
-- https://www.google.com/search?q=python+mail+merge+email&sxsrf=ALiCzsZaFP0_louBazSmaV2L5uFVcp1ceA%3A1651178446690&ei=zvtqYsXgKYnu_QbKpabgBw&ved=0ahUKEwjF_5mGz7f3AhUJd98KHcqSCXwQ4dUDCA4&uact=5&oq=python+mail+merge+email&gs_lcp=Cgdnd3Mtd2l6EAMyBwgjEK4CECcyBwgjEK4CECc6BwgjELADECc6BwgAEEcQsANKBAhBGABKBAhGGABQjAdYjAdg8wpoAXABeACAAW-IAW-SAQMwLjGYAQCgAQHIAQjAAQE&sclient=gws-wiz
-- 
--     https://kb.objectrocket.com/postgresql/email-with-postgres-and-python-1113
--     https://kb.objectrocket.com/postgresql/python-send-email-from-postgres-907
--     https://pythoninoffice.com/mail-merge-send-mass-email-in-python/
--     https://www.dataquest.io/blog/loading-data-into-postgres/
--     https://mailtrap.io/blog/python-mail-merge/
--     https://pypi.org/project/docx-mailmerge/

-- https://www.postgresqltutorial.com/postgresql-views/postgresql-materialized-views/#:~:text=PostgreSQL%20extends%20the%20view%20concept,to%20refresh%20this%20result%20periodically.
-- https://www.postgresql.org/docs/current/rules-materializedviews.html
-- https://www.postgresql.org/docs/current/sql-creatematerializedview.html
-- https://www.postgresql.org/docs/current/sql-altermaterializedview.html
-- https://www.postgresql.org/docs/current/sql-refreshmaterializedview.html
-- 
-- https://www.google.com/search?q=postgres+can+you+disable+refresh+of+materialized+view&oq=postgres+can+you+disable+refresh+of+mater&aqs=chrome.1.69i57j33i160j33i299l2.9185j0j7&sourceid=chrome&ie=UTF-8
-- https://orahow.com/how-to-stop-materialized-view-auto-refresh-in-oracle/#:~:text=A%20materialized%20view%20created%20with,aggregations%20of%20a%20table's%20data.
-- https://www.postgresql.org/docs/current/sql-refreshmaterializedview.html
-- https://www.postgresql.org/docs/14/sql-altermaterializedview.html
-- https://stackoverflow.com/questions/24413161/how-can-i-prevent-materialized-views-from-refreshing-during-pg-restore
-- 
-- https://www.google.com/search?q=international+phone+number+format&sxsrf=ALiCzsbE8uHKf52jUAn7Wpwy8HVUlTjzuA:1651258089138&tbm=isch&source=iu&ictx=1&vet=1&fir=PvDkF8EIcuNoDM%252CBIvXsNdXcp938M%252C_%253ByXL2hORCGXHDxM%252CJ4kyhGxZ2NeggM%252C_%253BBxyWNBMOxG5HSM%252ClNwZJvJrvBzCjM%252C_%253BV7NTqYbff8T8AM%252Ci_MBGgUZVjHYwM%252C_%253BQFuphORKHD0QqM%252Cz4JB8a1aR0BlaM%252C_%253BMQQokhlQ6oTrQM%252C4P3IuiFbdZB9jM%252C_%253BzbY6EHJVUzfXxM%252Cz4JB8a1aR0BlaM%252C_%253BlmMUvPm3OdlDsM%252CHDAJPENkxhuRcM%252C_%253BQbjSij5NlWs5FM%252CHmKOcDbJX_L4KM%252C_%253Bt3mAgQqYR7973M%252CUO_UVJvvqvu0rM%252C_&usg=AI4_-kTXfYAcCYKWOQqI7OlyAK5lcVRJJQ&sa=X&ved=2ahUKEwjbl9fe97n3AhXyQd8KHZFGAdMQ9QF6BAh0EAE
-- https://www.troyhunt.com/the-facebook-phone-numbers-are-now-searchable-in-have-i-been-pwned/
-- https://www.google.com/search?q=canada+internation+phone+format&oq=canada+internation+phone+format&aqs=chrome..69i57j0i22i30j0i390l2.8705j1j7&sourceid=chrome&ie=UTF-8
-- 
-- https://www.google.com/search?q=postgres+detect+what+called+a+function&oq=postgres+detect+what+called+a+function&aqs=chrome..69i57j33i22i29i30l5.9913j0j7&sourceid=chrome&ie=UTF-8
-- https://www.postgresql.org/message-id/CAM=VM83UZy_8EQDG5jOEEBN4x6qHuSYFu9Btr1CjF3QMU1QpAw@mail.gmail.com
-- https://www.postgresql.org/message-id/CAM=VM82+eTqLsLDG=bcO5-dY3MtX1Q+5VGTZjrGPKE1_y8LKpw@mail.gmail.com
-- 
-- https://stackoverflow.com/questions/859539/is-there-a-way-to-know-the-name-of-a-calling-function-in-pl-pgsql
-- 
-- https://www.google.com/search?q=can+you+lock+a+materialized+view+to+keep+from+refreshing&oq=can+you+lock+a+materialized+view+to+keep+from+refreshing&aqs=chrome..69i57j33i160.9898j0j7&sourceid=chrome&ie=UTF-8
-- 
-- https://www.google.com/search?q=how+to+have+a+function+return+true+or+false+postgres&sxsrf=ALiCzsbKJo8CaMwMoqaeKSDhPBYTdeJPcw%3A1651276370712&ei=UnpsYuT9Ku6g_QaxspWwDw&ved=0ahUKEwjk7oHsu7r3AhVuUN8KHTFZBfYQ4dUDCA8&uact=5&oq=how+to+have+a+function+return+true+or+false+postgres&gs_lcp=Cgdnd3Mtd2l6EAMyBQghEKABMgUIIRCgAToHCAAQRxCwA0oECEEYAEoECEYYAFDoAViRDWCpDWgBcAF4AIABrAGIAbYJkgEDMC44mAEAoAEByAEIwAEB&sclient=gws-wiz
-- https://www.postgresql.org/message-id/156905.9409.qm@web110714.mail.gq1.yahoo.com 
-- https://www.google.com/search?sxsrf=ALiCzsb0TvcTQxBjesi4-gZFsjeBErFHGw:1651290877874&q=use+subquery+to+get+table+name+then+using+table+name+list+in+from+to+perform+count+on+multiple+tables&spell=1&sa=X&ved=2ahUKEwj488jx8br3AhVtkeAKHShzCKMQBSgAegQIARAy&biw=1920&bih=947&dpr=1 
-- https://stackoverflow.com/questions/63726519/selecting-counts-from-different-tables-with-a-subquery 
-- https://stackoverflow.com/questions/15804797/count-the-number-of-rows-of-each-table-where-table-names-are-returned-from-subq 
-- https://stackoverflow.com/questions/3761240/count-from-multiple-tables-in-mysql 
-- https://learnsql.com/blog/sql-subquery-examples/ 
-- https://stackoverflow.com/questions/68754136/select-multiple-count-in-multiple-tables-with-single-query 
-- https://stackoverflow.com/questions/30293404/trying-to-use-subquery-to-get-row-count-from-another-table 
-- https://stackoverflow.com/questions/606234/select-count-from-multiple-tables
-- https://stackoverflow.com/questions/17194124/subqueries-to-select-multiple-counts-from-two-tables-in-one-query 
-- https://www.ibm.com/docs/en/qmf/11.2.1?topic=ddfmtuss-creating-subquery-retrieve-data-from-more-than-one-table
-- https://www.informit.com/articles/article.aspx?p=2036581&seqNum=9 
-- https://docs.teradata.com/r/Teradata-VantageTM-SQL-Data-Manipulation-Language/March-2019/Select-Statements/Correlated-Subqueries/Examples/Example-Using-SELECT-COUNT-in-a-Correlated-Subquery 
-- 
-- https://www.google.com/search?q=use+a+subquery+in+from+to+change+tables&oq=use+a+subquery+in+from+to+change+tables&aqs=chrome..69i57j33i160l3j33i22i29i30.8418j0j7&sourceid=chrome&ie=UTF-8
-- https://www.essentialsql.com/sql-subqueries/
-- https://learnsql.com/blog/converting-subqueries-joins/
-- https://docs.actian.com/vector/6.0/index.html#page/SQLLang/Subqueries_in_the_FROM_Clause_(Derived_Tables).htm
-- https://stackoverflow.com/questions/5931549/using-subquery-instead-of-the-tablename
-- 
-- https://www.google.com/search?q=TABLE_ROWS+postgres&oq=TABLE_ROWS+postgres&aqs=chrome..69i57j0i22i30l7j0i10i22i30j0i22i30.1583j0j7&sourceid=chrome&ie=UTF-8
-- https://stackoverflow.com/questions/5350088/how-to-search-a-specific-value-in-all-tables-postgresql
-- https://github.com/dverite/postgresql-functions/tree/master/global_search
-- 
-- https://stackoverflow.com/questions/62616716/find-number-of-rows-on-each-table-in-a-schema-in-postgres
-- https://stackoverflow.com/questions/7943233/fast-way-to-discover-the-row-count-of-a-table-in-postgresql
-- 
-- https://stackoverflow.com/questions/28667604/get-a-row-count-of-every-table-in-postgres-database
-- 
-- https://www.google.com/search?q=if+in+postgresql&oq=If+in+postgresql&aqs=chrome.0.0i20i263i512j0i512j0i10i22i30j0i22i30l2j69i60l3.2504j1j7&sourceid=chrome&ie=UTF-8
-- 
-- https://www.geeksforgeeks.org/postgresql-if-statement/
-- https://stackoverflow.com/questions/11299037/postgresql-if-statement
-- 
-- https://www.google.com/search?q=raise+notice+postgresql&oq=Raise+notice+postgres&aqs=chrome.0.0i20i263i512j69i57j0i67j0i512j0i67j0i512j0i22i30l4.3329j0j7&sourceid=chrome&ie=UTF-8
-- 
-- https://www.postgresql.org/docs/current/plpgsql-errors-and-messages.html
-- 
-- https://www.enterprisedb.com/postgres-tutorials/how-raise-errors-and-report-messages-within-stored-procedures-and-functions
-- 
-- https://www.google.com/search?q=print+statement+in+postgres&oq=print+in+postgres+&aqs=chrome.2.69i57j0i512j0i22i30l8.5248j0j7&sourceid=chrome&ie=UTF-8
-- https://stackoverflow.com/questions/23465429/printing-a-value-of-a-variable-in-postgresql
-- 
-- https://blog.mclaughlinsoftware.com/2019/10/12/postgres-print-debug-notes/
-- 
-- https://www.postgresql.org/docs/current/plpgsql-statements.html 
-- https://stackoverflow.com/questions/17547666/execute-into-using-statement-in-pl-pgsql-cant-execute-into-a-record
-- 
-- https://www.google.com/search?q=postgessql+table+count+comparision&oq=postgessql++table+count+comparision&aqs=chrome..69i57j0i13j0i13i30l2j0i8i13i30l6.14832j0j7&sourceid=chrome&ie=UTF-8
-- https://www.citusdata.com/blog/2016/10/12/count-performance/
-- https://vitessedata.com/blog/postgres-compare-table/
-- https://dwgeek.com/quick-best-way-compare-two-tables-sql.html/
-- https://blog.yugabyte.com/row-counts-of-tables-in-a-sql-schema-database-postgresql-yugabytedb/
-- 
-- https://www.postgresql.org/docs/current/functions-comparison.html
-- https://www.techonthenet.com/postgresql/comparison_operators.php
-- https://modern-sql.com/excel/countif-in-sql
-- https://www.mssqltips.com/sqlservertip/2779/ways-to-compare-and-find-differences-for-sql-server-tables-and-data/
-- https://www.oreilly.com/library/view/practical-postgresql/9781449309770/ch04s04.html
-- https://www.geeksforgeeks.org/postgresql-comparing-tables/
-- https://www.postgresqltutorial.com/postgresql-tutorial/compare-two-tables-in-postgresql/
-- https://dba.stackexchange.com/questions/195795/postgres-sql-compare-count-of-rows-returning-where-a-b-or-b-doesnt-exist
-- 
-- https://www.google.com/search?q=postgres+table+type+in+information_schema&sxsrf=ALiCzsZfSQgAoSu2voNCApc7RDHzReB9og%3A1651353324189&ei=7KZtYoWhC5Lr_QbqqoywBg&ved=0ahUKEwiFx6XC2rz3AhWSdd8KHWoVA2YQ4dUDCA8&uact=5&oq=postgres+table+type+in+information_schema&gs_lcp=_CiSAQUyOC4yNZgBAKABAcABAQ&sclient=gws-wiz
-- 
-- https://razorsql.com/articles/postgresql_system_queries.html
-- https://www.ronaldjamesgroup.com/blog/sql-201-6-ways-to-search-with-system-tables
-- https://www.ananunesdasilva.com/posts/finding-all-tables-with-a-specific-column-name-postgres
-- https://dba.stackexchange.com/questions/75015/get-column-names-and-data-types-of-a-query-table-or-view
-- https://www.orafaq.com/forum/t/163672/0/
-- https://www.datavail.com/blog/implementing-countdistinct-with-fast-refresh-materialized-views/
-- 
-- https://docs.snowflake.com/en/user-guide/views-materialized.html
-- https://www.postgresql.org/docs/current/infoschema-tables.html
-- https://www.postgresql.org/docs/current/information-schema.html
-- 
-- https://stackoverflow.com/questions/2276644/list-all-tables-in-postgresql-information-schema
-- https://stackoverflow.com/questions/58431104/difference-between-information-schema-tables-and-pg-tables
-- 
-- https://www.google.com/search?q=postgres+function+parameter+optional+amount+of+parameters&sxsrf=ALiCzsb0FN9cR6ddRx3Qk1x0VCnbJk-0Gw%3A1651368169199&ei=6eBtYvS1C4-6ggfroYqQDA&ved=0ahUKEwi0zvjokb33AhUPneAKHeuQAsIQ4dUDCA8&uact=5&oq=postgres+function+parameter+optional+amount+of+parameters&gs_lcp=Cgdnd3Mtd2l6EAMyBQghEKABOgcIABBHELADOgcIIxCwAhAnOgUIIRCrAkoECEEYAEoECEYYAFCkBljLMmDkMmgBcAF4AYAB9AGIAcIfkgEGMC4yNy4ymAEAoAEByAEIwAEB&sclient=gws-wiz
-- https://stackoverflow.com/questions/39896329/how-to-write-function-for-optional-parameters-in-postgresql
-- https://www.geeksforgeeks.org/postgresql-function-parameters/
-- 
-- https://www.google.com/search?q=VARIADIC+postgres&sxsrf=ALiCzsb7XUJPWAGKxZfZfuOXy0AK4t5QTA%3A1651368213249&ei=FeFtYtjnDqOd_Qb2lIGQDA&ved=0ahUKEwiYxvn9kb33AhWjTt8KHXZKAMIQ4dUDCA8&uact=5&oq=VARIADIC+postgres&gs_lcp=Cgdnd3Mtd2l6EAMyCggAEIAEEIcCEBQyBQgAEIAEMgYIABAWEB4yBggAEBYQHjIGCAAQFhAeMgUIABCGAzIFCAAQhgMyBQgAEIYDOgcIABBHELADOgcIABCwAxBDOgQIABBDOggIABAWEAoQHkoECEEYAEoECEYYAFA9WKQWYL0WaARwAXgBgAGMAYgBpwiSAQMwLjmYAQCgAQHIAQrAAQE&sclient=gws-wiz
-- https://forum.lazarus.freepascal.org/index.php?topic=42651.0
-- https://postgrespro.com/list/thread-id/2514847
-- https://blog.yugabyte.com/polymorphism-in-sql-variadic-functions/
-- 
-- https://www.google.com/search?sxsrf=ALiCzsaWNd96dFoIg3-y-UPDGry45bvseg:1651368471270&q=variadic+postgresql&spell=1&sa=X&ved=2ahUKEwiT7v34kr33AhUJd98KHVwICTgQirwEKAB6BAgCEDI&biw=1920&bih=947&dpr=1
-- https://stackoverflow.com/questions/33802583/what-does-variadic-declarations-mean-in-postgresql
-- https://www.postgresonline.com/journal/archives/211-Variadic-Functions-in-PostgreSQL.html
-- 
-- https://en.wikipedia.org/wiki/Variadic_function
-- https://www.postgresql.org/docs/current/typeconv-func.html
-- https://www.postgresql.org/docs/current/xfunc-sql.html
-- https://academy.vertabelo.com/course/user-defined-functions/writing-functions/function-parameters/variadic-parameters
-- 
-- https://dba.stackexchange.com/questions/158078/how-to-use-an-array-as-argument-to-a-variadic-function-in-postgresql
-- https://stackoverflow.com/questions/59091412/postgresql-function-with-optional-parameters
-- https://www.postgresql.org/docs/9.1/functions-string.html
-- https://www.postgresql.org/docs/current/plpgsql-declarations.html
-- https://stackoverflow.com/questions/59091412/postgresql-function-with-optional-parameters
-- 
-- https://stackoverflow.com/questions/38442106/postgresql-check-if-table-exist-throwing-relation-does-not-exist-error
-- 
-- https://dba.forumming.com/question/9320/sql-error-42883-function-does-not-exist-cannot-call-function-from-trigger
-- https://www.google.com/search?sxsrf=ALiCzsYbOmiHfC11qgrdfR0idi5m7g9KdQ:1651456981248&q=materialized+sql+%2242883%22+error&sa=X&ved=2ahUKEwjQrOrV3L_3AhUKh-AKHbv9A2EQ5t4CegQIThAB&biw=1920&bih=947&dpr=1
-- 
-- https://www.google.com/search?q=drop+trigger+postgres&sxsrf=ALiCzsaUG65usmQ_z9K5kmqfmUSNYJ-mrA%3A1651536221878&ei=XXFwYsSDNcel_QaHqoiwAQ&oq=drop+triggers+&gs_lcp=Cgdnd3Mtd2l6EAEYATIFCAAQgAQyBAgAEAoyBAgAEAoyBAgAEAoyBAgAEAoyBAgAEAoyBAgAEAoyBAgAEAoyBAgAEAoyBAgAEAo6BwgAEEcQsANKBAhBGABKBAhGGABQIFggYMEGaAFwAXgAgAGHAYgBhwGSAQMwLjGYAQCgAQHIAQjAAQE&sclient=gws-wiz
-- https://www.guru99.com/postgresql-trigger-create-drop.html#:~:text=To%20drop%20a%20PostgreSQL%20trigger,that%20is%20to%20be%20deleted.
-- 
-- https://www.postgresql.org/docs/current/sql-droptrigger.html
-- https://www.postgresqltutorial.com/postgresql-triggers/postgresql-drop-trigger/
-- https://www.geeksforgeeks.org/postgresql-drop-trigger/?ref=rp
-- https://dataschool.com/learn-sql/errors/
-- 
-- https://stackoverflow.com/questions/39896329/how-to-write-function-for-optional-parameters-in-postgresql
-- https://stackoverflow.com/questions/11519410/optional-argument-in-pl-pgsql-function
-- 
-- https://stackoverflow.com/questions/59091412/postgresql-function-with-optional-parameters
-- https://quick-adviser.com/how-do-you-pass-optional-parameters-in-postgresql-function/
-- https://dba.stackexchange.com/questions/233822/default-values-in-function-arguments-dependency-postgresql
-- https://www.geeksforgeeks.org/postgresql-function-parameters/
-- https://stackoverflow.com/questions/39896329/how-to-write-function-for-optional-parameters-in-postgresql
-- 
-- https://www.google.com/search?q=postgres+function+optional+parameter+if+blank+ignore&oq=postgres+function+optional+parameter+if+blank+ignore&aqs=chrome..69i57j33i160l2j33i299.10129j0j7&sourceid=chrome&ie=UTF-8
-- https://stackoverflow.com/questions/17353445/test-for-null-in-function-with-varying-parameters
-- 
-- https://stackoverflow.com/questions/39896329/how-to-write-function-for-optional-parameters-in-postgresql
--
-- https://ebookcentral.proquest.com/lib/westerngovernors-ebooks/reader.action?docID=5888693&ppg=164
-- https://ebookcentral.proquest.com/lib/westerngovernors-ebooks/reader.action?docID=5888693&ppg=164#ppg=138
-- 
-- https://popsql.com/learn-sql/postgresql/how-to-write-a-case-statement-in-postgresql
-- https://www.w3resource.com/PostgreSQL/pl-pgsql-control-structures.php
-- https://www.enterprisedb.com/postgres-tutorials/how-work-control-structures-postgresql-stored-procedures-using-if-case-and-loop
-- https://dba.stackexchange.com/questions/162540/set-variable-result-in-case-statement-to-be-used-again-in-the-same-select-statem
-- https://www.geeksforgeeks.org/postgresql-case-statement/
-- https://stackoverflow.com/questions/71324689/select-into-a-variable-with-a-case-expression
-- https://stackoverflow.com/questions/59379786/assign-results-from-query-to-variable-for-use-in-case-statement
-- 
-- https://stackoverflow.com/questions/8924035/postgresql-case-usage-in-functions
-- https://social.msdn.microsoft.com/Forums/sqlserver/en-US/0fc9df95-ee22-4570-affd-c89217eb15e9/sql-case-when-with-variable?forum=transactsql
-- https://social.msdn.microsoft.com/Forums/sqlserver/en-US/0c338d7a-f173-4fb3-81f5-e191363541e8/how-to-set-a-variable-in-case-statement?forum=transactsql 
-- https://social.msdn.microsoft.com/Forums/sqlserver/en-US/698eaddd-8098-40b4-8db9-54397e51d4f6/set-a-variable-using-a-case-expression-inside-a-select-statement?forum=transactsql
-- 
-- https://bobcares.com/blog/postgresql-error-42601/

-- https://www.postgresql.org/docs/current/trigger-definition.html
-- https://qtibia.com/how-to-install-the-pl-pgsql-debugger-and-use-it-in-pgadmin/
-- https://chartio.com/resources/tutorials/how-to-define-an-auto-increment-primary-key-in-oracle/
-- https://stackoverflow.com/questions/13048360/reading-current-table-id-during-trigger-operation
-- https://dba.stackexchange.com/questions/144197/postgresql-use-new-in-query-for-instead-of-trigger
-- https://medium.com/analytics-vidhya/interesting-5-functions-and-5-triggers-with-postgresql-52700d554192
-- https://stackoverflow.com/questions/47852776/how-can-i-fire-a-trigger-at-the-end-of-a-chain-of-updates
-- https://stackoverflow.com/questions/11001118/postgres-trigger-after-insert-accessing-new
-- https://stackoverflow.com/questions/70145624/how-to-trigger-after-update-in-postgres
-- https://stackoverflow.com/questions/62333675/how-to-tell-if-when-postgres-triggers-are-executing
-- https://stackoverflow.com/questions/306439/debugging-postgresql-trigger
-- https://stackoverflow.com/questions/28994033/postgresql-trigger-fire-on-update-but-fail-on-insert

-- https://www.cockroachlabs.com/docs/stable/common-table-expressions.html
-- https://www.adoclib.com/blog/dbeaver-sql-common-expression-table-question.html
-- https://dbeaver.io/forum/viewtopic.php?f=2&t=669
-- 
-- https://stackoverflow.com/questions/22746741/trigger-for-checking-a-given-value-before-insert-or-update-on-postgresql
-- https://stackoverflow.com/questions/4069718/postgres-insert-if-does-not-exist-already
-- 
-- https://www.postgresqltutorial.com/postgresql-tutorial/postgresql-random-range/

-- https://stackoverflow.com/questions/10687582/postgresql-trigger-not-working-neither-before-nor-after-delete
-- https://www.bluelabellabs.com/blog/how-to-use-postgresql-trigger-functions-to-automate-creation-and-last-modification-timestamps/
-- 
-- https://stackoverflow.com/questions/50038611/pgadmin4-enable-debugging
-- https://qtibia.com/how-to-install-the-pl-pgsql-debugger-and-use-it-in-pgadmin/
-- https://serveanswer.com/questions/set-breakpoint-function-in-postgresql-pgadmin

-- https://codebeautify.org/sqlformatter

-- https://wiki.postgresql.org/wiki/Transactional_DDL_in_PostgreSQL:_A_Competitive_Analysis
-- https://stackoverflow.com/questions/35927365/create-or-replace-trigger-postgres
-- https://stackoverflow.com/questions/45579643/trigger-error-create-trigger
-- https://www.postgresql.org/docs/13/sql-createtrigger.html
-- https://phoenixnap.com/kb/check-postgresql-version#:~:text=Type%20the%20following%20SQL%20statement,information%20for%20the%20PostgreSQL%20server.
-- https://stackoverflow.com/questions/11299037/postgresql-if-statement
-- https://www.google.com/search?q=sql+state+42601+postgresql&oq=sql+state+42601+postgresql+&aqs=chrome..69i57j0i22i30l2j0i390l4.12313j0j7&sourceid=chrome&ie=UTF-8
-- https://www.postgresql.org/docs/8.1/errcodes-appendix.html
-- https://www.postgresql.org/message-id/9700A92FD044934E8E64B214D8B0F38C1C1B0BDA55@MAILBOX1.nrel.gov
-- https://stackoverflow.com/questions/16291944/postgresql-sql-state-42601-syntax-error

-- https://stackoverflow.com/questions/61886884/postgresql-add-auto-increment-to-empty-id-column
-- https://www.postgresql.org/docs/current/ddl-alter.html#:~:text=To%20set%20a%20new%20default,default%20for%20future%20INSERT%20commands.

-- https://www.2ndquadrant.com/en/blog/postgresql-10-identity-columns/
-- https://www.postgresql.org/docs/current/sql-altertable.html
-- https://www.postgresqltutorial.com/postgresql-tutorial/postgresql-primary-key/
-- https://stackoverflow.com/questions/2944499/how-to-add-an-auto-incrementing-primary-key-to-an-existing-table-in-postgresql
-- https://www.cybertec-postgresql.com/en/sequences-gains-and-pitfalls/
-- https://popsql.com/learn-sql/postgresql/how-to-alter-sequence-in-postgresql


-- https://stackoverflow.com/questions/61886884/postgresql-add-auto-increment-to-empty-id-column
-- https://www.postgresql.org/docs/current/ddl-alter.html#:~:text=To%20set%20a%20new%20default,default%20for%20future%20INSERT%20commands.

-- https://stackoverflow.com/questions/55664506/reset-identity-column-with-last-value-of-tables-identity-in-postgres
-- https://pgpedia.info/p/pg_get_serial_sequence.html
-- https://www.postgresql.org/docs/8.2/functions-sequence.html
-- https://stackoverflow.com/questions/55664506/reset-identity-column-with-last-value-of-tables-identity-in-postgres
-- https://stackoverflow.com/questions/244243/how-to-reset-postgres-primary-key-sequence-when-it-falls-out-of-sync
-- https://stackoverflow.com/questions/24756035/postgresql-increase-a-tables-sequence-with-one-query
-- https://stackoverflow.com/questions/68207140/how-can-i-increment-a-sequence-by-more-than-1-in-postgres
-- https://stackoverflow.com/questions/68668001/is-setval-nextval-increment-on-postgresql-sequence-thread-safe

-- https://www.postgresql.org/docs/current/sql-createtablespace.html
-- https://www.postgresql.org/docs/current/manage-ag-tablespaces.html

-- https://www.postgresql.org/docs/13/sql-createindex.html
-- https://docs.oracle.com/cd/B19306_01/server.102/b14220/physical.htm
-- https://www.cybertec-postgresql.com/en/when-to-use-tablespaces-in-postgresql/#:~:text=What%20is%20a%20tablespace,has%20its%20own%20data%20file.
-- https://www.cybertec-postgresql.com/en/json-postgresql-how-to-use-it-right/
-- https://www.cybertec-postgresql.com/en/understanding-lateral-joins-in-postgresql/
-- https://www.cybertec-postgresql.com/en/entity-attribute-value-eav-design-in-postgresql-dont-do-it/
-- https://www.cybertec-postgresql.com/en/query-parameter-data-types-performance/
-- https://www.cybertec-postgresql.com/en/postgresql-creating-checksums-for-tables/
-- https://www.cybertec-postgresql.com/en/automatic-partition-creation-in-postgresql/
-- https://www.cybertec-postgresql.com/en/postgresql-the-power-of-a-single-missing-index/

-- https://stackoverflow.com/questions/35184509/make-selected-block-of-text-uppercase


-- https://www.postgresql.org/docs/current/sql-createindex.html
-- https://www.postgresqltutorial.com/postgresql-indexes/postgresql-create-index/
-- https://www.tutorialspoint.com/postgresql/postgresql_indexes.htm
-- https://popsql.com/learn-sql/postgresql/how-to-create-an-index-in-postgresql
-- https://www.postgresqltutorial.com/postgresql-tutorial/postgresql-coalesce/
-- https://www.postgresql.org/docs/current/functions-conditional.html
-- https://www.geeksforgeeks.org/postgresql-coalesce/
-- https://www.enterprisedb.com/postgres-tutorials/how-use-coalesce-postgresql

-- https://www.postgresql.org/docs/current/sql-dropindex.html
-- https://www.geeksforgeeks.org/postgresql-drop-index/
-- https://www.oracletutorial.com/oracle-index/oracle-drop-index/

-- https://stackoverflow.com/questions/40654766/how-to-increase-max-locks-per-transaction



-- https://www.crunchydata.com/blog/optimize-postgresql-server-performance
-- https://stackoverflow.com/questions/67016945/what-is-the-downside-to-increase-shared-buffer-in-postgresql
-- https://www.cybertec-postgresql.com/en/are-triggers-really-that-slow-in-postgres/
-- https://www.postgresql.org/docs/current/routine-vacuuming.html
-- https://www.postgresql.org/files/developer/concurrency.pdf
-- https://tapoueh.org/blog/2018/07/postgresql-listen-notify/
-- https://www.citusdata.com/blog/2018/02/22/seven-tips-for-dealing-with-postgres-locks/
-- https://www.reddit.com/r/PostgreSQL/comments/mgnfn2/at_what_scale_will_postgresql_slow_down/
-- https://www.reddit.com/r/PostgreSQL/comments/sf0bi8/80_million_records_how_to_handle_it/
-- https://www.2ndquadrant.com/en/services-old/postgresql-performance-tuning/
-- https://www.postgresql.org/docs/9.3/using-explain.html#USING-EXPLAIN-ANALYZE
-- https://wiki.postgresql.org/wiki/Performance_Optimization
-- https://www.enterprisedb.com/postgres-tutorials/introduction-postgresql-performance-tuning-and-optimization
-- https://stackify.com/postgresql-performance-tutorial/

-- https://www.citusdata.com/blog/2018/06/12/configuring-work-mem-on-postgres/
-- https://aws.amazon.com/blogs/database/tune-sorting-operations-in-postgresql-with-work_mem/
-- https://www.pgmustard.com/blog/work-mem
-- https://www.enterprisedb.com/postgres-tutorials/how-tune-postgresql-memory


-- https://stackoverflow.com/questions/24193567/for-each-statement-trigger-example
-- https://www.postgresql.org/docs/current/plpgsql-trigger.html
-- https://www.postgresql.org/docs/current/sql-createtrigger.html
-- https://www.postgresql.org/docs/current/trigger-definition.html
-- https://www.enterprisedb.com/postgres-tutorials/can-modified-tuples-be-accessed-using-statement-level-trigger-postgres
-- https://www.postgres-xl.org/documentation/plpgsql-trigger.html
-- https://www.ibm.com/docs/en/db2/11.5?topic=dt-referencing-old-new-table-result-sets-using-transition-tables
-- https://www.geeksforgeeks.org/difference-between-row-level-and-statement-level-triggers/
-- https://arctype.com/blog/learn-sql-triggers/
-- https://stackoverflow.com/questions/32210225/postgresql-run-trigger-after-update-for-each-statement-only-if-data-changed
-- https://stackoverflow.com/questions/57432855/how-can-a-postgres-trigger-after-update-for-each-statement-correlate-its-old
-- https://stackoverflow.com/questions/61682957/postgresql-trigger-before-insert-for-each-statement
-- https://dba.stackexchange.com/questions/230500/what-is-a-practical-use-of-for-statement-triggers
-- https://www.depesz.com/2012/11/14/how-i-learned-to-stop-worrying-and-love-the-triggers/
-- https://www.postgresql.org/docs/current/plpgsql-trigger.html
-- https://www.postgresql.org/docs/current/triggers.html
-- https://www.postgresql.org/docs/current/trigger-datachanges.html



-- https://www.ibm.com/docs/en/db2/10.5?topic=dt-specifying-what-makes-trigger-fire-triggering-statement-event
-- https://www.ibm.com/docs/en/db2/10.5?topic=dt-specifying-when-trigger-fires-before-after-instead-clauses
-- https://www.ibm.com/docs/en/db2/10.5?topic=dt-defining-conditions-when-trigger-action-will-fire-when-clause
-- https://www.ibm.com/docs/en/db2/10.5?topic=triggers-supported-sql-pl-statements-in
-- https://www.ibm.com/docs/en/db2/10.5?topic=dt-accessing-old-new-column-values-in-triggers-using-transition-variables
-- https://www.ibm.com/docs/en/db2/10.5?topic=dt-referencing-old-new-table-result-sets-using-transition-tables
-- https://www.ibm.com/docs/en/db2/10.5?topic=triggers-examples-trigger-use
-- 
-- https://stackoverflow.com/questions/42920998/pl-pgsql-perform-vs-execute
-- https://www.postgresql.org/docs/current/plpgsql-statements.html
-- https://stackoverflow.com/questions/67709708/pl-pgsql-perform-vs-call
-- https://www.postgresql.org/docs/current/plpgsql-statements.html
-- https://www.w3resource.com/PostgreSQL/pl-pgsql-basic-statements.php
-- https://www.linuxtopia.org/online_books/database_guides/Practical_PostgreSQL_database/PostgreSQL_x20238_004.htm
-- https://www.educba.com/postgresql-execute/

-- https://www.enterprisedb.com/postgres-tutorials/can-modified-tuples-be-accessed-using-statement-level-trigger-postgres
-- https://infocenter.sybase.com/help/index.jsp?topic=/com.sybase.help.sqlanywhere.12.0.1/dbreference/create-trigger-statement.html
-- https://www.careerride.com/Oracle-row-level-and-statement-level-trigger.aspx
-- https://livesql.oracle.com/apex/livesql/file/content_CTTF4QDB9N3CY6PU1GKN3B444.html
-- https://www.tutorialspoint.com/plsql/plsql_triggers.htm
-- https://www.postgresql.org/docs/current/sql-createtrigger.html


-- https://wiki.postgresql.org/wiki/Tuning_Your_PostgreSQL_Server
-- https://www.enterprisedb.com/postgres-tutorials/introduction-postgresql-performance-tuning-and-optimization
-- https://www.enterprisedb.com/blog/improving-postgresql-performance-without-making-changes-postgresql
-- https://www.percona.com/blog/2018/08/31/tuning-postgresql-database-parameters-to-optimize-performance/
-- https://marmelab.com/blog/2019/02/13/how-to-improve-postgres-performances.html
-- https://wiki.postgresql.org/wiki/Performance_Optimization
-- https://www.craigkerstiens.com/2012/10/01/understanding-postgres-performance/
-- https://marmelab.com/blog/2018/04/03/how-to-track-and-fix-memory-leak-with-nodejs.html



-- https://helperbyte.com/questions/411022/what-is-the-cause-of-the-error-out-of-shared-memory-you-might-need-to-increase-max_locks_per_transaction
-- https://www.cybertec-postgresql.com/en/postgresql-you-might-need-to-increase-max_locks_per_transaction/

-- https://www.postgresql.org/docs/current/view-pg-locks.html
-- https://www.postgresql.org/docs/current/explicit-locking.html
-- https://dba.stackexchange.com/questions/300722/out-of-shared-memory-and-lock-monitoring-in-postgresql
-- https://dba.stackexchange.com/questions/185880/analyze-memory-usage-of-postgresql-why-is-it-growing-constantly/270183#270183

-- https://stackoverflow.com/questions/6898453/how-to-display-the-function-procedure-triggers-source-code-in-postgresql
-- https://stackoverflow.com/questions/12611596/getting-name-of-the-current-function-inside-of-the-function-with-plpgsql



-- #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####      
-- #### #### #### #### #### #### #### #### #### #### #### ####         RESOURCE LINKS END         #### #### #### #### #### #### #### #### #### #### #### #### ####      
-- #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####                           
